...
def sample(
    ...
    y_gen = D.y_transform.inverse_transform(y_gen.reshape(-1, 1)).reshape(-1)
    np.save(os.path.join(parent_dir, 'y_train'), y_gen)

if __name__ == "__main__":
    p = Path("C:/Users/trist/OneDrive/Dokumente/UZH/BA/06_Code/TabDDPM/exp/adult/ddpm_mlp_best")
    x_cat_unnorm = p / 'X_cat_unnorm.npy'
    x_cat_train = p / 'X_cat_train.npy'
    x_num_unnorm = p / 'X_num_unnorm.npy'
    x_num_train = p / 'X_num_train.npy'
    y_train = p / 'y_train.npy'

    xcatunnorm = np.load(x_cat_unnorm)
    xcattrain = np.load(x_cat_train, allow_pickle=True)
    xnumunnorm = np.load(x_num_unnorm)
    xnumtrain = np.load(x_num_train, allow_pickle=True)
    ytrain = np.load(y_train, allow_pickle=True)

    xcatunnorm_pd = pd.DataFrame(xcatunnorm)
    xcattrain_pd = pd.DataFrame(xcattrain)
    xnumunnorm_pd = pd.DataFrame(xnumunnorm)
    xnumtrain_pd = pd.DataFrame(xnumtrain)
