...
group_stats = pd.read_csv(os.path.join(tabddpm_results_dir, f"{model_type}-{it}-group-stats.csv"))
...
tabddpm_df = pd.read_csv(os.path.join(tabddpm_results_dir, f"{model_type}-{it}-agg-across-groups.csv"))
ctgan_df = pd.read_csv(os.path.join(ctgan_results_dir, f"ctgan-300-agg-across-groups.csv"))
...
out_path_total = os.path.join(tabddpm_dir, "plots", f"bar_plot_all.png")
os.makedirs(os.path.dirname(out_path_total), exist_ok=True)
bar_plot_total(tabddpm_df, baseline_tabddpm_df, ctgan_df, baseline_ctgan_df, out_path_total, metrics, f"Comparison of Metrics across Models ({dataset_name.capitalize()})")

repo_root = get_repo_root()
fedtabdiff_dir = repo_root / 'exp' / 'adult' / 'runs' / 'FedTabDiff'
...
key_col = results_4.columns[0]

# Merge on the key column
fedtabdiff_df = pd.concat(fedtabdiff_dict.values())
# For each metric, get mean + std
fedtabdiff_agg = {}
for metric in metrics:
    metric_values = fedtabdiff_df[metric].values
    fedtabdiff_agg[metric] = {
        'Mean': np.mean(metric_values),
        'Std': np.std(metric_values, ddof=1)
    }
fedtabdiff_agg_df = pd.DataFrame(fedtabdiff_agg).T
fedtabdiff_agg_df.reset_index(inplace=True)
fedtabdiff_agg_df.rename(columns={'index': 'Metric'}, inplace=True)
fedtabdiff_agg_df.to_csv(fedtabdiff_dir / 'fedtabdiff_agg.csv', index=False)


tabddpm_df = pd.read_csv(os.path.join(tabddpm_results_dir, f"{model_type}-{it}-group-stats.csv"))
out_path_bar = os.path.join(tabddpm_dir, "plots", f"bar_plot_fedtabdiff.png")
os.makedirs(os.path.dirname(out_path_bar), exist_ok=True)
bar_plot_fedtabdiff(tabddpm_df, fedtabdiff_dict, out_path_bar, metrics, f"Comparison of Metrics: DeConSyn-TabDDPM vs FedTabDiff ({dataset_name.capitalize()})")
