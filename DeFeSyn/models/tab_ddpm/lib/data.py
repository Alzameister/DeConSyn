...
def y_encode(
    ...
    if encoding is None:
        ...
        encoder.fit(y['train'].reshape(-1, 1))
        y = {k: encoder.transform(v.reshape(-1, 1)).astype('float32') for k, v in y.items()}  # type: ignore[code]
...
def transform_dataset(
    ...
    if dataset.n_classes > 0:
        # Transform
        y = cat_process_nans(dataset.y, transformations.cat_nan_policy)
        if transformations.cat_min_frequency is not None:
            y = cat_drop_rare(y, transformations.cat_min_frequency)
    ...
    dataset.y_transform = y_transform
